...
取消/暂停竞态回归门：`http://127.0.0.1:8765/tests/recorder-sim.html` —— 真实 Chromium 里 mock 音频硬件面，驱动播放录制通道 6 条路径（自然结束 / 中途取消 / 意外暂停 / 停滞收尾 / 解锁成功 / 解锁取消），任何一条卡死即 FAIL。

## 版本

当前 **8.1.2**（v8.1.2：取消「保留已转写内容」落到实处 + recorderAsr 状态机模拟器 6 路径防回归门；v8.1.1：修复播放录制兜底通道的取消/暂停竞态——解锁死锁、意外暂停永久挂起、取消后脏转写、消费者异常静默；v8.1：解决「音频解码失败（可能内存不足）」，MIME 自愈 + 内存预检 + 播放录制兜底 + 真实 SF key 冒烟）。
